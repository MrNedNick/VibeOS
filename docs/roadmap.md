...
4. **UI Kit component library** ✅ complete (v1.0.12), **moved under Docs + made public (v1.2.0)**
   - Built `src/modules/ui-kit/views/sections/` + `ShowcaseCard`, `PropTable`, `PgStage`, `TokenSwatch`, `TokenRow`.
   ...
   - **v1.2.0:** removed the standalone hidden `/ui-kit` route + `UiKitView` shell + 4-pak theme switcher. The sections are now wired into the Docs module via `src/modules/docs/data/docs-registry.ts` as three sidebar groups (Design Tokens / UI Components / UI Patterns) at `/docs/ui-kit/<key>`. `DocPage.component` renders a live showcase; `DocsView` falls back to markdown otherwise. **Visible in production** — no longer dev-only.
   - Original spec ↓
...
### T4 — Component tests for @/ui primitives 🧩 ✅ (22/22 — v2.7.21)

All 22 @/ui components now have test coverage.

**Progress (complete — 22/22):** `UiButton`, `UiCard`, `UiFilterChips`, `UiIconButton`, `UiModal`, `UiSelect`, `UiTextarea` (S17 Phase 0) + `UiInput`, `UiField`, `UiProgressBar`, `UiBadge`, `UiEmptyState` (v2.7.7) + `UiFab`, `UiStat`, `UiSectionLabel`, `UiProgressRing`, `UiSkeleton` (v2.7.8) + `UiConfirmDialog`, `UiFeedbackModal` (v2.7.9) + **`UiIcon`, `UiToastContainer`, `UiPlannedView`** (v2.7.21 — icon resolver renders svg/nothing for valid/invalid names; toast container shows/dismisses all 4 type variants, caps at 3; planned view renders all props + router.push back navigation).

---

### T5 — Component tests for refactored god-components 🎯 🔄 (v2.7.10)
...
**Progress:** Analytics panels (`analyticsComponents.test.ts`) + `BoardCard`, `HabitCardCalendar`, `HabitCardLinks` (v2.7.10 — 36 cases) + **`FinanceTransactions`, `FinanceBudgets`** (v2.7.11 — 14 cases) + **`BoardColumn`** (v2.7.21 — 13 cases) + **`StudioHistorySidebar` + `StudioModelPicker`** (v2.7.22 — 21 cases) + **`StudioConversation` + `FinanceOverview`** (v2.7.23 — 30 cases: StudioConversation: empty-state title per provider, quick-prompts, no-key warning, user/assistant/error message classes, typing indicator, send button; FinanceOverview: empty state text/button, breakdown-seg/cat-row counts, budget progress bar, AI button disabled state, AI result card show/hide/dismiss). **Remaining: `TimelineGrid` (419 LOC, complex drag grid — deferred).**
...
- **T6 — task-manager + notes** ✅ (v1.2.2) — `UiInput.focus()` exposed; Notes: `UiIconButton` for new-note + dismiss, `UiButton` for all toolbar actions (Back/AI/Today/Export/Delete/empty-state), bespoke mode-toggle + backlinks kept (structural); task-manager: `UiIconButton` for AI dismiss, `UiSelect` for goal picker, `UiFilterChips` for Pomodoro presets + modes, `UiButton` for Start/Resume, `UiIconButton` for Reset, `UiSelect` for focus task; S9 fix in TaskInput priority CSS (rgba → color-mix, hex → CSS vars); dead CSS deleted from all touched files. Exceptions documented: NoteEditor textarea (full-panel editor), mode-toggle buttons (bespoke active state), backlinks (navigation), task checkbox + inline edit input (structural bespoke elements), category-color badges (semantic module constants).
- **T7 — habits + goals** ✅ (v1.2.3) — goals: `UiButton` for all actions (add/cancel/form/completed-toggle/empty-state), `UiInput`+`UiField`+`UiSelect` for form fields (emoji input kept bespoke), `MilestoneList` migrated (`UiInput`+`UiButton`+`UiIconButton` for delete, opacity-0 trick preserved), `GoalDetailView` migrated (`UiButton`/`UiIconButton`/`UiInput`/`UiTextarea` for notes/tasks, Mark-complete kept bespoke for success-color outline, AI suggestions kept bespoke for dashed-border layout, task circle-check kept bespoke); habits: `UiButton` for add/save/cancel/empty/templates, `UiIconButton` for milestone-close + AI-dismiss, `UiSelect` for goal picker, `UiProgressBar` for weekly bar (dynamic success/accent/warning color), bespoke kept: form emoji+name+purpose inputs (borderless inline), category filter chips (--cat CSS var + toggle-deselect), HabitCard deferred (1100+ LOC, separate pass). All dead CSS removed from touched files.
- **T8 — learning + training** ✅ (v1.2.4) — SessionLogForm + WorkoutLogForm: full UiModal conversion (replaced position:fixed overlay + panel with UiModal size="sm", UiInput for text fields, UiTextarea for notes, UiButton for actions, UiIconButton for close; number/star/emoji inputs kept bespoke); plan cards: UiButton for Log action, UiProgressBar for LearningPlanCard; LearningView + TrainingView: UiButton for all actions, UiInput for title/AI-prompt, UiSelect for sport, UiIconButton for AI-close; both PlanDetailViews: UiButton for back/log/add-resource/save-habit/delete, UiIconButton for AI-dismiss/resource-delete, UiInput for resource form, UiSelect for habit link; resource type icons + done-toggle kept bespoke. All dead CSS removed from touched files.
- **T9 — finance + kanban** ✅ (v1.2.5) — finance: UiModal for add-expense overlay (replaced Teleport+fixed overlay), UiButton for Add/CSV/empty-state, UiIconButton for month nav/AI-dismiss/tx-delete/budget-save, UiSelect for base+display currency, UiInput for currency symbol, S9 fix (hardcoded hex budget colors → CSS vars); kanban: UiModal for add-card overlay, UiInput+UiTextarea+UiSelect for modal form, UiButton for add-card/modal-actions, UiIconButton for card-delete/task-panel-close/search-clear; bespoke kept: view-toggle buttons (tab navigation), priority chips, mobile tabs, card expand/priority-cycle/date/move-to (structural card widgets), inline desc textarea (inline editing), recurring toggle (emoji widget), category grid (per-category color).
- **T10 — dashboard** ✅ (v1.2.6) — DashboardView: UiButton for customize-widgets + module-row launch; DashboardWidgetCustomizer: UiButton for reset, UiIconButton for close + eye-toggle; HabitsPanel: UiButton for view-all + empty-state CTA; GoalsPanel: UiButton for view-all + empty-state CTA; AllTasksPanel: UiButton for shipped-toggle. Bespoke kept: habit-row circular toggle.
- **T11 — settings + ai-playground** ✅ (v1.2.6) — Settings: UiInput for API key, UiButton for show/hide + export + import + confirm + cancel + danger/clear; bespoke kept: pak-cards, language toggle, module vis-toggle; Studio: UiButton for clear-history + new-chat + system + copy, UiIconButton for sidebar-item-delete + sidebar-toggle + key-eye; bespoke kept: tab buttons, model chips, context-toggle, system/chat textareas (specialized chat widgets).
- **T12 — analytics + calendar + docs + about** ✅ (v1.2.6) — DocsView: UiButton for "Start reading" + "Back to index". Analytics/Calendar/About: no raw elements found.
- **T13 — auth + welcome** ✅ (v1.2.6) — LoginView + RegisterView: UiInput for all email/password fields, UiButton for all submit/demo/text-link buttons; WelcomeView: UiButton for nav Sign-in + Try-demo CTA + hero primary/ghost CTAs; bespoke kept: tab buttons, module card grid (navigation cards with per-card color).
