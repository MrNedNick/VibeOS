...
| **S16 — Test Coverage** | Store/composable unit tests, component tests, smoke E2E, manual QA pass | 🔄 active — T1–T3 ✅ T6 ✅ T8 ✅; T4 🔄 (12/22 @/ui components); remaining: T5, T7 QA pass (needs live review). **473 tests in 39 files** |
...
### T4 — Component tests for @/ui primitives 🧩 🔄 (12/22 — v2.7.7)
...
**Progress (12/22 components covered):** `UiButton`, `UiCard`, `UiFilterChips`, `UiIconButton`, `UiModal`, `UiSelect`, `UiTextarea` (S17 Phase 0) + **`UiInput`, `UiField`, `UiProgressBar`, `UiBadge`, `UiEmptyState`** (v2.7.7 — 43 cases: v-model/error/enter-emit/exposed-focus, label+hint/error+required+slot, fill-width clamping+color variants+aria, variant classes, icon-vs-emoji+action emit). Remaining untested (mostly presentational): `UiSectionLabel`, `UiStat`, `UiProgressRing`, `UiSkeleton`, `UiPlannedView`, `UiIcon`, `UiToastContainer`, `UiFab`, `UiFeedbackModal`, `UiConfirmDialog` (the last two are interaction-heavy — next priority).
