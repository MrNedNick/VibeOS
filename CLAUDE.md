...
Vitest v4 + happy-dom. 369 tests in 27 files. `npm test` = run once. Coverage gate: stmt 35% / branch 22%. Playwright E2E: `e2e/smoke.spec.ts`. For Teleport components (UiModal) → query `document.body`, not `wrapper.find()`.
